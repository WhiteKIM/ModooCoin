...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("ModooCoin"))
        return false;
...
{
    QString ret = QString("ModooCoin:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ModooCoin.desktop";
    return GetAutostartDir() / strprintf("ModooCoin-%s.lnk", chain);
}
