 ...
 */
static const char* BITCOIN_PID_FILENAME = "ModooCoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("ModooCoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ModooCoin-project/ModooCoin>";
    const std::string URL_WEBSITE = "<https://ModooCoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("ModooCoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if ModooCoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if ModooCoin is started while in a temporary directory.\n",
